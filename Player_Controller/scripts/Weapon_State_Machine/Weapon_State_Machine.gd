...
signal Weapon_Changed(weapon: Weapon_Resource)
signal Update_Ammo
...
@onready var Bullet_Casing = preload("res://Player_Controller/Spawnable_Objects/BulletCase/bullet_case.tscn")
@onready var Ejection_Marker = get_node("%EjectionMarker")
...
func shoot():
	if Current_Weapon.Current_Ammo != 0:
		...
		if not Animation_Player.is_playing():
			...
			if Current_Weapon.Projectile_Type == "Hitscan":
				Eject_Bullet_Casing()			
...
func Eject_Bullet_Casing():
	var casing = Bullet_Casing.instantiate()
	get_tree().get_root().add_child(casing)

	# Set the initial position of the casing to the EjectionMarker
	casing.global_transform = Ejection_Marker.global_transform

	# Calculate the right direction in world space
	var right_direction = Ejection_Marker.global_transform.basis.x

	# Calculate ejection direction (right and slightly upward)
	var ejection_direction = (right_direction + Vector3.UP * 0.5).normalized()

	# Apply force in the ejection direction
	var ejection_force = ejection_direction * randf_range(3, 5)
	casing.apply_central_impulse(ejection_force)

	# Apply a small random torque for rotation
	casing.apply_torque(Vector3(randf(), randf(), randf()) * 0.2)

	# Set up a timer to remove the casing after a few seconds
	var timer = get_tree().create_timer(8.0)
	timer.timeout.connect(func(): casing.queue_free())
...
func _on_pick_up_detection_body_entered(body):
	pass
	if body.TYPE == "Weapon":
		var Weapon_In_Stack = WeaponStack.find(body._weapon_name)
		
		if Weapon_In_Stack != -1:
			# Weapon is already in the stack
			var weapon_resource = Weapons_List[body._weapon_name]
			
			if weapon_resource.Projectile_To_Load.instantiate().Projectile_Type == "Rigidbody_Projectile":
				# For Rigidbody_Projectile, add directly to current ammo
				var total_ammo = body._current_ammo + body._reserve_ammo
				var max_current_ammo = weapon_resource.Max_Ammo
				var added_ammo = min(total_ammo, max_current_ammo - weapon_resource.Current_Ammo)
				
				weapon_resource.Current_Ammo += added_ammo
				
				if added_ammo == total_ammo:
					# All ammo was added, remove the pickup
					body.queue_free()
				else:
					# Some ammo couldn't be added, update the pickup
					body.update_ammo(total_ammo - added_ammo, 0)
			else:
				# For other weapon types, use the existing logic
				var total_ammo = body._current_ammo + body._reserve_ammo
				var remaining = Add_Ammo(body._weapon_name, total_ammo)
				...
				if remaining == 0:
					...
					# Some ammo couldn't be added, update the pickup
					body.update_ammo(min(remaining, weapon_resource.Magazine), 
									 max(remaining - min(remaining, weapon_resource.Magazine), 0))
			
			# Update ammo display if it's the current weapon
			if body._weapon_name == Current_Weapon.Weapon_Name:
				Update_Ammo.emit([Current_Weapon.Current_Ammo, Current_Weapon.Reserve_Ammo])
		
		elif body.Pick_Up_Ready == true:
			# Weapon is not in stack, add it
			var GetRef = WeaponStack.find(Current_Weapon.Weapon_Name)
			WeaponStack.insert(GetRef, body._weapon_name)
			
			# Set ammo for the new weapon
	...
	# Handle other types of pickups if necessary
...
func Add_Ammo(_Weapon: String, Ammo: int) -> int:
	...
	if _Weapon == Current_Weapon.Weapon_Name:
		Update_Ammo.emit([Current_Weapon.Current_Ammo, Current_Weapon.Reserve_Ammo])
